...
- Motion system: copy is intentionally minimal — motion carries the story. `AnimatedText` reveals headings word-by-word (blur+lift), `Parallax` scrubs elements as they scroll, `Reveal` does enter animations, and `useSmoothScroll` is a dependency-free inertia scroller (iOS-style glide) that drives `ScrollTrigger.update()`.
- The 3D hero is `Centerpiece` (`src/three/Centerpiece.tsx`): a custom GLSL object (Ashima simplex-noise displacement + fresnel rim) that morphs/recolors with scroll and reacts to the pointer; the `NeuralNetwork` is now the ambient field behind it, and `CameraRig` orbits/dollies around the centerpiece. Shared pointer state is `src/three/pointer.ts`.
- Desktop-only flourishes self-disable on touch / reduced-motion and key off `(pointer: fine)` + `(hover: hover)`: custom cursor (`Cursor`), magnetic hovers (`Magnetic`), kinetic heading skew (`useKineticScroll` → `--vskew` on `.kinetic`), and smooth scroll. Per-scene accent is the `--accent` CSS var set from the active chapter in `App.tsx`.
